import { normalizeAffiliateSource, summarizeAffiliateEvents, validateAffiliateEvent } from "./affiliate-events.mjs";
...
export function combineClickDays(days) {
  ...
  for (const day of days) for (const row of day.rows) {
    const source = normalizeAffiliateSource(row.source);
    const key = JSON.stringify([row.page, row.product, row.campaign, row.placement, source]);
    const previous = groups.get(key) || { page: row.page, product: row.product, campaign: row.campaign, placement: row.placement, source, clicks: 0 };
...
export async function saveDailyReport(clickStore, reportStore, now = new Date(), { deadline = Date.now() + 25000 } = {}) {
  ...
  const snapshot = {
    schemaVersion: 2, generatedAt: now.toISOString(), timezone: "UTC",
    from: dates[0], to: dates.at(-1), status: "complete",
    metric: "recorded affiliate click events; not unique visitors, bookings or revenue",
    sourceScope: "Allowlisted utm_source on the page where the click occurred only; no cross-page attribution. Unspecified includes untagged, invalid and older events.",
