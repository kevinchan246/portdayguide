...
- Choose at most one useful, source-backed improvement per week within the existing acquisition and booking paths. Prioritize a confirmed breakage, incorrect product match, or an improvement justified by real data. The Cozumel taxi guide is the first data-backed addition to the original Tokyo focus. Review recent merged work before repeating an edit; allow a complete measurement window instead of churning titles or layouts. If evidence is insufficient, complete monitoring and record that fact; do not generate articles to meet a quota.
