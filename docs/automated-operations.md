...
3. **Weekly operating agent:** use the separate ChatGPT scheduled task after its successful creation. Fetch this document from current `main` on each run. Use GitHub and public HTTP, both verified for this task. Private services only become dependencies after a successful harmless authenticated read. Do not claim any connection grants access to a report until a read succeeds.
...
- Choose at most one useful, source-backed improvement per week within the first experiment. Prioritize a confirmed breakage, incorrect product match, or an improvement justified by real data. If evidence is insufficient, complete monitoring and record that fact; do not generate articles to meet a quota.
...
- GSC access is pending; do not manufacture a current baseline from the old July–August export. Viator booking and commission reports are not connected by the product API. Establish an authenticated export/integration separately.
