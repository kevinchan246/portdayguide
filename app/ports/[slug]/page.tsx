...
function itinerary(profile: (typeof profilesBySlug)[PortSlug], hours: 6 | 8) {
  ...
  return [
    ...
    { time: `${formatOffset(mainStart)}–${formatOffset(mainEnd)}`, title: profile.slug === "osaka" ? "One city area: castle or food district" : profile.highlights[0], text: profile.slug === "osaka" ? "Choose Osaka Castle, Dotonbori or Shinsekai. Keep this block in the chosen area; traveling from the castle to Dotonbori is a separate journey. Kyoto is not part of this plan." : hours === 6 ? "Keep this as the one main experience." : `Add ${profile.highlights[1]} only when it is genuinely nearby and transport is running comfortably.` },
    { time: `${formatOffset(mainEnd)}–${formatOffset(leaveFinalStop)}`, title: profile.slug === "osaka" ? "Flexible time in the same area" : "Flexible final stop on the return route", text: profile.slug === "osaka" ? "Use this time for the same visit, a nearby food stop or an earlier return. Do not commit it to another district without rebuilding the full transport plan." : "Use a short food or waterfront stop only when it does not require a detour." },
