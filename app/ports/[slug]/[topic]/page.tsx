...
import { IntentBookingLinks } from "@/components/IntentBookingLinks";
  ...
  return <main className="intent-guide-page">
    ...
    <article className={`intent-article${isYokohamaTerminalArticle ? " intent-editorial-article" : ""}`}>
      {isYokohamaTerminalArticle ? <YokohamaTerminalArticle guide={guide} hub={hub} /> : <>
        <section className="intent-answer" aria-labelledby="quick-answer-title"><span>Quick answer</span><h2 id="quick-answer-title">{guide.quickAnswerHeading || "The decision in one minute"}</h2><p>{guide.quickAnswer}</p><IntentBookingLinks portSlug={guide.sourcePortSlug} topic={guide.topic} /><div>{guide.facts.map((fact) => <dl key={fact.label}><dt>{fact.label}</dt><dd>{fact.value}</dd></dl>)}</div></section>
