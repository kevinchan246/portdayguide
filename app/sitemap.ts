...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const guides = portNames.map((name) => ({ url: `${siteUrl}${portPath(portProfiles[name].slug)}`, lastModified: portContentUpdate(portProfiles[name].slug)?.modified ?? updated, changeFrequency: "monthly" as const, priority: .8 }));
